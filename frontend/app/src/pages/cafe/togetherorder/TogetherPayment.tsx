...
import useWebSocket, {OrderItemPayload, OrderRequestDto} from "../../../api/useWebSocket.tsx";
...
function TogetherPayment() {
  ...
  const executeOrder = () => {
    const menuList = products.map((product): OrderItemPayload => ({
  ...
  return (
    <div className="mb-20">
          ...
          return (
            ...
            >
              <div className="flex-1">
                <span className="text-base font-hyemin-bold">{item.menuname}</span>
                {options.length > 0 && (
      ...
      {products.length === 0 && <p className="text-center">장바구니가 비어 있습니다.</p>}
